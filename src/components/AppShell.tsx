...
import {
  getRegistrarModuleById,
  getRegistrarModuleByPath,
  registrarModules,
  type RegistrarModule,
} from "@/lib/registrar-modules";

// ── Server function: logout ───────────────────────────────────────────────────

const logoutFn = createServerFn({ method: "POST" }).handler(async () => {
  const { clearSessionCookie } = await import("@/lib/auth.server");
...
const titles: Record<string, { title: string; sub: string }> = {
  ...
  "/academic-levels": {
    ...
    sub: "DEPED-aligned academic level structure",
  },
    ...
    <div className="min-h-screen bg-gradient-subtle">
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-72 flex-col border-r border-[#d7e0ee] bg-[#f7f9fc] text-[#101827] shadow-[10px_0_36px_-28px_rgba(15,23,42,0.45)] lg:flex">
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-5">
          <NavGroup
            label="MVP Modules"
            modules={registrarModules.filter((m) => m.group === "MVP")}
            pathname={pathname}
          />
          <NavGroup
            label="Production Add-ons"
            modules={registrarModules.filter((m) => m.group === "Add-on")}
            pathname={pathname}
            offset={10}
          />
        </nav>
          ...
          <div className="flex h-16 items-center gap-3 px-4 sm:px-6 lg:px-8">
            ...
            <div className="hidden flex-1 items-center md:flex">
              <div className="group relative w-full max-w-md">
                ...
                />
                <kbd className="absolute right-3 top-1/2 hidden -translate-y-1/2 rounded border border-border bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground md:inline">
                  ⌘K
                </kbd>
              </div>
            </div>
    ...
    <div className="border-b border-[#d7e0ee] bg-white/80 px-4 py-4 backdrop-blur">
      <div className="flex items-center gap-3 rounded-2xl border border-[#d7e0ee] bg-white px-3 py-3 shadow-soft">
        ...
        <div className="min-w-0">
          <div className="font-display text-base font-semibold tracking-tight text-[#101827]">
            Scholaris
          </div>
...
function NavGroup({
  ...
  label: string;
  modules: RegistrarModule[];
  pathname: string;
  offset?: number;
}) {
  return (
    <div className="mb-5">
      <div className="mb-2 px-2 text-[10px] font-bold uppercase tracking-[0.2em] text-[#b45309]">
        {label}
      </div>
      <div className="space-y-1.5">
      ...
      >
        ...
        <span
          className={cn(
            ...
            active
              ? "bg-[#101827] text-[#f5c84b]"
              : "bg-white text-[#64748b] ring-1 ring-[#d7e0ee] group-hover:text-[#101827]"
          )}
        >
        ...
        {module.status === "Blueprint" && (
          <span
            className={cn(
              ...
              active ? "bg-white text-[#b45309]" : "bg-[#e9eef6] text-[#64748b]"
            )}
          >
            Plan
          </span>
    ...
    <div className="border-t border-[#d7e0ee] bg-white/60 p-4">
      <div className="rounded-2xl border border-[#d7e0ee] bg-white p-3 shadow-soft">
        <div className="flex items-center gap-2.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#3658df] text-sm font-semibold text-white shadow-[0_12px_24px_-14px_rgba(54,88,223,0.8)]">
            {user.initials}
          </div>
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold text-[#101827]">
              {user.name}
            </div>
        ...
        <button
          ...
          className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-xl border border-[#d7e0ee] bg-[#f8fafc] px-3 py-2 text-xs font-semibold text-[#101827] transition-colors hover:border-red-200 hover:bg-red-50 hover:text-red-600"
        >
